...
export class BlobService {
  ...
  async updateBlob(blob: IBlob, id: string): Promise<IBlob | undefined> {
    ...
    if (blob.users) {
      // Delete all existing relationships for the Blob
      const deleteAllRelationshipsQuery = `
                MATCH (user:User)-[r:BELONGS_TO]->(blob:Blob)
                WHERE blob.uuid = $id
                DETACH DELETE r
            `;
    ...
    params.selectedUserIds = blob.users;
    ...
    const createRelationshipsQuery = `
            MATCH (blob:Blob), (user:User)
            WHERE blob.uuid = $id AND user.uuid IN $selectedUserIds
            MERGE (user)-[:BELONGS_TO]->(blob)
        `;
